...
location = '/mnt/c/Users/rohith/Desktop/FLstudio/pop/audio/wav/'
name = 'pop_1_130BPM.wav'
...
chord_labels = chunk_send.chunk_sender(peaks=peaks, z_boundaries=z_boundaries, aud=y,method = 'cqt', name=name.split('.')[0],hop_length= 512,binsPerOctave= 36,sr=44100)

#5. Convert the frame based chord labels to time based labels
est_file = feature.frame_file_converter(frame_array=chord_labels , sr=sr, hop_length= hop_length, dest_path = location, name =name.split('.')[0])

#6. Compare annotations 
ref_file = '/mnt/c/Users/rohith/Desktop/FLstudio/pop/annotation/chords/convertedAnnotation/Timed_pop_1_130BPM.txt'
...
visual.plot_both_onsetzero(sr,y,times,peaks,z,z_boundaries, o_env_max= 1.5*y.max())
